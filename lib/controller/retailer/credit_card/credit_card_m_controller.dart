...
import '../../../repository/retailer/credit_card/credit_card_m_repository.dart';
import '../../../utils/string_constants.dart';
import '../../../widgets/constant_widgets.dart';

class CreditCardMController extends GetxController {
  CreditCardMRepository creditCardMRepository = CreditCardMRepository(APIManager());
  ...
  Future<bool> getBankListAPI({bool isLoaderShow = true}) async {
    try {
      List<RecipientDepositBankModel> bankModel = await creditCardMRepository.getBankApiCall(
  ...
  Future<bool> transactionSlab({bool isLoaderShow = true}) async {
    try {
      transactionSlabModel.value = await creditCardMRepository.transactionSlabApiCall(
        params: {
          'gatewayName': 'MOBIKWIK',
  ...
  Future<int> transaction({required String amount, required int slabNo, bool isLoaderShow = true}) async {
    try {
      transactionModel.value = await creditCardMRepository.transactionApiCall(
        params: {
          'gatewayName': 'MOBIKWIK',
          'slno': slabNo,
          'mobileNo': mobileNumberController.text.trim(),
