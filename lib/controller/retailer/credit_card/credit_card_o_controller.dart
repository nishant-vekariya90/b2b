...
import '../../../repository/retailer/credit_card/credit_card_o_repository.dart';
import '../../../utils/string_constants.dart';
import '../../../widgets/constant_widgets.dart';

class CreditCardOController extends GetxController {
  CreditCardORepository creditCardORepository = CreditCardORepository(APIManager());
  ...
  Future<bool> getBankListAPI({bool isLoaderShow = true}) async {
    try {
      List<RecipientDepositBankModel> bankModel = await creditCardORepository.getBankApiCall(
  ...
  Future<bool> transactionSlab({bool isLoaderShow = true}) async {
    try {
      transactionSlabModel.value = await creditCardORepository.transactionSlabApiCall(
        params: {
          'gatewayName': 'OFFLINE',
  ...
  Future<int> transaction({required String amount, required int slabNo, bool isLoaderShow = true}) async {
    try {
      transactionModel.value = await creditCardORepository.transactionApiCall(
        params: {
          'gatewayName': 'OFFLINE',
          'slno': slabNo,
          'mobileNo': mobileNumberController.text.trim(),
