...
import '../../repository/retailer/bbps_repository.dart';
import '../../utils/string_constants.dart';
import '../../widgets/constant_widgets.dart';

class BbpsController extends GetxController {
  BbpsRepository bbpsRepository = BbpsRepository(APIManager());
  ...
  Future<bool> getBbpsSubCategoryList({required int serviceId, bool isLoaderShow = true}) async {
    try {
      List<BbpsSubCategoryListModel> bbpsSubCategoryListModel = await bbpsRepository.getBbpsSubCategory(
        id: serviceId,
        isLoaderShow: isLoaderShow,
      );
  ...
  Future<bool> getBbpsParametersFieldList({required int operatorId, bool isLoaderShow = true}) async {
    try {
      List<BbpsParametersListModel> bbpsParametersListModel = await bbpsRepository.getBbpsFieldList(
        id: operatorId,
        isLoaderShow: isLoaderShow,
      );
  ...
  Future<bool> getBbpsOperatorGroupingList({required int operatorId, required int operatorParamId, bool isLoaderShow = true}) async {
    try {
      List<BbpsOperatorGroupingListModel> bbpsOperatorGroupingListModel = await bbpsRepository.getMasterBbpsOperatorGrouping(
        operatorId: operatorId,
  ...
  Future<bool> fetchBbpsBill({required String operatorCode, bool isLoaderShow = true}) async {
    try {
      fetchBillModel.value = await bbpsRepository.fetchBillDetails(
        params: {
      ...
      if (fetchBillModel.value.statusCode == 1) {
        ...
        customerName.value = fetchBillModel.value.data!.billerName!;
        return true;
  ...
  Future<int> payBbpsBill({required String operatorCode, bool isLoaderShow = true}) async {
    try {
      billPaymentModel.value = await bbpsRepository.getPayBbpsBill(
        params: {
          'operatorCode': operatorCode,
          ...
          'accountNumber': valueControllers.length >= 2 ? valueControllers[1].text.toString() : '',
          'authenticator': valueControllers.length >= 3 ? valueControllers[2].text.toString() : '',
          'customerName': customerName.isNotEmpty?customerName.value.trim().toString():'',
          'mobileNo': agentMobileNumberController.text.trim(),
          'dueDate': fetchBillModel.value.data==null?'':fetchBillModel.value.data!.billDueDate!.toString(),
          'orderId': 'App${DateTime.now().millisecondsSinceEpoch.toString()}',
          'billRefNo': fetchBillModel.value.data==null?'':fetchBillModel.value.data!.billRefNumber!.toString(),
          'tpin': tPinController.text.isNotEmpty ? tPinController.text : '',
          'channel': channelID,
          'ipAddress': ipAddress,
          'latitude': latitude,
